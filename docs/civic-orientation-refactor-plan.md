...
- TODO: Explore route-level progressive hydration after the guided path is visually stable. Load the shell, property identity, current step, and next-step orientation immediately; defer heavier later-step modules/data such as market charts, equalization charts, tax treemaps, county comparison, and report/PDF setup until the user reaches or is about to reach those steps. Cache rendered/prepared steps after first use so back/forward navigation stays instant.
- TODO: After mobile, tablet, and desktop views are locked, sweep for visually identical or near-identical components and consolidate their markup/CSS where practical. Prioritize shared metric cards, voluntary disclosure controls, drawers, chart containers, source-note treatments, and section headers; keep legitimate page-specific exceptions only when the content model requires them.
- TODO: After layout lock, split the current large catch-all files into clearer modules without changing behavior. Start with `src/charts.js`, `src/render.js`, and `src/styles.css`; preserve the current route/data boundaries, then move repeated card, drawer, source-note, KPI, chart, and guided-transition patterns into reusable modules or style sections.
