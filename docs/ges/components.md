...
Purpose: provide compact categorical labels, filters, status indicators, and small navigation chips without changing size across breakpoints.

Contract: use the shared pill geometry tokens. Pills use 12px UI type, a 16px line box, 26px minimum block size, 10px inline padding, nowrap labels, and row wrapping only between pills.

Use for: `article-entry-tags`, `ges-tag`, `ges-pill`, Article Roll filters, public section-nav chips, Article Roll status labels, and older comparison-review badges while those remain in the legacy app stylesheet.

Do not use: custom per-component font sizes, content-driven vertical padding, viewport-scaled pill type, or mobile-only shrink rules. Preserve color/state variants, but keep the geometry stable.

Mobile: pills may wrap as items in the row. A single pill label should not wrap internally; use concise labels rather than allowing multi-line pills.

## Inner Cards / List Tiles

Purpose: keep small repeated cards inside a component from changing size, alignment, or density by accident.

Contract: use the shared inner-card tokens for radius, gap, padding, and compact tile height. Simple list tiles, such as `assessment-build-panel li`, use a stable 3.5rem minimum block size with vertically centered text. Richer inner cards, such as comparison lanes, question checklist items, resource cards, and record-callout rows, use the shared radius/gap/padding while their content determines final height.

Use for: small repeated items inside article components, including assessment build fields, record-callout rows, comparison/scorecard lanes, question checklist items, resource cards, and similar nested instructional tiles.

Do not use: viewport-scaled padding, route-specific mini-card radii, text-driven vertical rhythm, or mobile-only sizing overrides. If a tile label is too long, shorten the label before allowing the component to become a multi-line card grid.

Mobile: tile grids may reduce column count, but the tile treatment remains the same. Text aligns vertically inside each tile, and wrapping happens at item boundaries first.
...
Draft previews: internal users may open draft cards when the manifest entry has a real preview route. Draft cards with only future metadata remain non-clickable to avoid 404 paths. Use `route.previewable: true` when a draft has a route and also needs a `route.sourceNote`; use `route.previewable: false` to intentionally suppress a draft link.
...
## Section Divider / Paper-Fold Crease

Purpose: mark the boundary between adjacent article story sections without adding authored content.

Use when: a `tax-story-chapter` directly follows another `tax-story-chapter`. The crease is generated by CSS through the universal adjacent-section rule.

Do not use when: the article is opening its first section, or when a normal spacing transition is enough. Authors should not insert an explicit `hr`, `renderPageCrease()`, or local crease component.

Behavior: the crease appears above the later section and before its heading or Margin Insight row. It is a decorative full-width two-line paper-fold treatment, not a semantic divider.

Accessibility: generated decoration only; do not rely on it to communicate section order.

Mobile/print: keep the same section-boundary meaning. Print rules may flatten the visual treatment, but the section order remains normal document order.
...
Placement: Margin Insights may sit in the right margin at the top of a section, stack directly below a section heading, or appear below body content. The text is red-900 in light mode and a green evidence tone in dark mode. The label/header does not inherit this color treatment.

First section exception: do not add a crease above the first story section. There is no preceding body content to separate, so the crease adds noise.

Authoring: prefer the shared `renderMarginInsight()` / `renderSectionHeader()` helpers. Do not use the removed `first` API or `ges-margin-insight--first` class. Future article data may set `placement: "after-content"` on a Margin Insight when it is intentionally placed after prose instead of inside a section header.

Mobile: collapses into the reading column. Adjacent-section creases remain decorative and should not create a second reading path.
...
Directional tint: use `comparison-card--directional` only when the lanes describe directionality, such as "hard to act on" versus "easier to evaluate." The yellow/green fills are intentionally subtle and must not imply success, failure, approval, denial, or legal outcome.
...
Article note renderer: use `renderArticleNote()` for caution and guidance notes in article routes. Caution and guidance notes share the same compact font size at every breakpoint; only accent color and icon role should differ.
...
Punctuation rule: semicolons, periods, and other citation punctuation stay inside the rendered source-note link/span using `article-source-note__punctuation`. Do not leave punctuation as a separate text node before the next link icon.
...
Behavior: render with `renderContinuationModule()`. The shared renderer supports normal article continuations and compact related-article codas through options for class, heading tag, id, and analytics action.
