...
Adjacent story chapters use the universal paper-fold crease rule: `.tax-story-chapter + .tax-story-chapter::before`.

This treatment divides major article sections after prior content. It is generated by CSS, not authored with an `hr` or route helper. The first story section must not receive the crease because there is no preceding article section to separate.

The crease belongs to section rhythm, not to Margin Insight. A section may have a Margin Insight and receive a crease, but those are independent concerns: the crease marks the boundary; the insight orients the reader inside the new section.

Mobile and print should preserve the same source order. The visual rule may flatten in print, but authors should not insert alternate print-only dividers.
