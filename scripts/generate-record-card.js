...
function usage() {
  console.error([
    ...
    "       node scripts/generate-record-card.js /path/to/parcel.pdf [--sample-visibility research]",
    "",
...
function updateManifest(recordIdValue, recordCardPath, pdf, options = {}) {
  const manifestPath = path.join(process.cwd(), "data/app/property-manifest.json");
  const manifest = JSON.parse(fs.readFileSync(manifestPath, "utf8"));
  const existingEntry = manifest.properties.find(property => property.id === recordIdValue);
  const entry = {
    id: recordIdValue,
    ...
    sampleVisibility: options.sampleVisibility || existingEntry?.sampleVisibility || "public"
  };
